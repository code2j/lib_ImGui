...
void UpdateModelAnimation(Model model, ModelAnimation anim, float frame)
{
    ...
    if ((anim.keyframeCount > 0) && (model.skeleton.bones != NULL) && (anim.keyframePoses != NULL))
    {
        ...
        for (int boneIndex = 0; boneIndex < model.skeleton.boneCount; boneIndex++)
        {
            ...
            //-----------------------------------------------------------------------------------
            Transform_ *bindPoseTransform = &model.skeleton.bindPose[boneIndex];
            ...
            Transform_ *currentPoseTransform = &model.currentPose[boneIndex];
...
static void BuildPoseFromParentJoints(BoneInfo *bones, int boneCount, Transform_ *transforms)
{
...
static Model LoadIQM(const char *fileName)
{
    ...
    */
    ...
    model.skeleton.bindPose = (Transform_ *)RL_CALLOC(iqmHeader->num_joints, sizeof(Transform_));
    ...
    model.currentPose = (Transform_ *)RL_CALLOC(model.skeleton.boneCount, sizeof(Transform_));
...
static ModelAnimation *LoadModelAnimationsIQM(const char *fileName, int *animCount)
{
    ...
    *animCount = iqmHeader->num_anims;
    ...
    for (unsigned int a = 0; a < iqmHeader->num_anims; a++)
    {
        ...
        animations[a].keyframePoses = (Transform_ **)RL_CALLOC(anim[a].num_frames, sizeof(Transform_ *));
        ...
        for (unsigned int j = 0; j < anim[a].num_frames; j++)
            animations[a].keyframePoses[j] = (Transform_ *)RL_MALLOC(iqmHeader->num_poses*sizeof(Transform_));
    ...
    ***********************************************************************************************/
    ...
    if (result == cgltf_result_success)
    {
        ...
        if (data->skins_count > 0)
        {
            ...
            model.skeleton.bindPose = (Transform_ *)RL_CALLOC(model.skeleton.boneCount, sizeof(Transform_));
        ...
        if (data->skins_count > 0)
        {
            ...
            *animCount = (int)data->animations_count;
            ...
            for (unsigned int a = 0; a < data->animations_count; a++)
            {
                ...
                animations[a].keyframePoses = (Transform_ **)RL_CALLOC(animations[a].keyframeCount, sizeof(Transform_ *));

                for (int j = 0; j < animations[a].keyframeCount; j++)
                {
                    animations[a].keyframePoses[j] = (Transform_ *)RL_CALLOC(animations[a].boneCount, sizeof(Transform_));
    ...
    if (fileData != NULL)
    {
        ...
        *animCount = m3d->numaction;
        ...
        for (unsigned int a = 0; a < m3d->numaction; a++)
        {
            ...
            {
                animations[a].keyframePoses[i] = (Transform_ *)RL_CALLOC((m3d->numbone + 1), sizeof(Transform_));
