...
typedef Transform_ *ModelAnimPose;
...
RLAPI RayFont GetFontDefault(void);                                                            // Get the default Font
RLAPI RayFont LoadFont(const char *fileName);                                                  // Load font from file into GPU memory (VRAM)
RLAPI RayFont LoadFontEx(const char *fileName, int fontSize, const int *codepoints, int codepointCount); // Load font from file with defined codepoints and generation size, use NULL for codepoints and 0 for codepointCount to load the default character set, font size is provided in pixels height
RLAPI RayFont LoadFontFromImage(Image image, Color key, int firstChar);                        // Load font from Image (XNA style)
RLAPI RayFont LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, const int *codepoints, int codepointCount); // Load font from memory buffer, fileType refers to extension: i.e. '.ttf'
RLAPI bool IsFontValid(RayFont font);                                                          // Check if font is valid (font data loaded, WARNING: GPU texture not checked)
...
RLAPI bool ExportFontAsCode(RayFont font, const char *fileName);                               // Export font as code file, returns true on success
...
RLAPI void DrawTextCodepoints(RayFont font, const int *codepoints, int codepointCount, Vector2 position, float fontSize, float spacing, Color tint); // Draw multiple characters (codepoint)
...
RLAPI Vector2 MeasureTextCodepoints(RayFont font, const int *codepoints, int length, float fontSize, float spacing); // Measure string size for an existing array of codepoints for Font
...
RLAPI Rectangle GetGlyphAtlasRec(RayFont font, int codepoint);                                 // Get glyph rectangle in font atlas for a codepoint (unicode character), fallback to '?' if not found
